...
def main(args):
        ...
        if args.run_name == 'auto':
            if len(args.train_devices) < 5:
                if args.stride is not None:
                    args.run_name = f'{args.backbone}_with_SAM_stride_{args.stride}_integrated_except_{"_".join(target_device)}_{args.stego_method}_{args.batch_size}_{args.lr}'
                else:
                    args.run_name = f'{args.backbone}_with_SAM_integrated_except_{"_".join(target_device)}_{args.stego_method}_{args.batch_size}_{args.lr}'
            else:
                if args.stride is not None:
                    args.run_name = f'{args.backbone}_with_SAM_integrated_{args.data_type}_{args.total_data}_stride_{args.stride}_{args.stego_method}_{args.batch_size}_{args.lr}'
                else:
                    args.run_name = f'{args.backbone}_with_SAM_integrated_{args.data_type}_{args.total_data}_{args.stego_method}_{args.batch_size}_{args.lr}'
            if args.suffix != '':
    ...
    train_num = int(args.total_data * train_rate) // len(args.train_devices)
    valid_num = int(args.total_data * valid_rate) // len(args.train_devices)

    train_df = pd.DataFrame()
    train_ipp_df = pd.DataFrame()
    valid_df = pd.DataFrame()
    for train_device in args.train_devices:
        device_df = pd.read_csv(f'{args.csv_root}/single/{args.data_type}/{args.cover_size}_{args.stego_method}/{train_device}_train.csv')[:train_num]
        valid_df = pd.concat([valid_df, pd.read_csv(f'{args.csv_root}/single/{args.data_type}/{args.cover_size}_{args.stego_method}/{train_device}_valid.csv')[:valid_num]])
        
        if args.apply_ipp:
            train_df = pd.concat([train_df, device_df[:len(device_df)//2]], ignore_index=True)
            ipp_df = pd.read_csv(f'{args.csv_root}/single/{args.data_type}/{args.cover_size}_ipp_from_cover_v2_{args.stego_method}/{train_device}_train.csv')[:train_num]
            train_ipp_df = pd.concat([train_ipp_df, ipp_df[:len(ipp_df)//2]], ignore_index=True)
        else:
            train_df = pd.concat([train_df, device_df], ignore_index=True)

    if args.apply_ipp:
        train_df = pd.concat([train_df, train_ipp_df], ignore_index=True)
    ...
    print(f'=================================== integrated model with SAM training ===================================')
    best_valid_acc = 0.0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--total_data', type=int, default=100000, help='Total number of data')
    ...
    if args.use_wandb:
        ...
        wandb.save(f'./train_integrated_SAM.py', policy='now')

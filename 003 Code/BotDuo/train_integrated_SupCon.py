        ...
        if args.run_name == 'auto':
            if len(args.train_devices) < 5:
                if args.stride is not None:
                    args.run_name = f'{args.backbone}_SupCon_stride_{args.stride}_integrated_except_{"_".join(target_device)}_{args.stego_method}_{args.batch_size}_{args.lr}'
                else:
                    args.run_name = f'{args.backbone}_SupCon_integrated_except_{"_".join(target_device)}_{args.stego_method}_{args.batch_size}_{args.lr}'
            else:
                if args.stride is not None:
                    args.run_name = f'{args.backbone}_SupCon_{int(args.ce_loss_weight*10)}_{int((1-args.ce_loss_weight)*10)}_integrated_{args.data_type}_{args.original_data+args.ipp_data}_stride_{args.stride}_{args.stego_method}_{args.batch_size}_{args.lr}'
                else:
                    args.run_name = f'{args.backbone}_SupCon_{int(args.ce_loss_weight*10)}_{int((1-args.ce_loss_weight)*10)}_integrated_{args.data_type}_{args.original_data+args.ipp_data}_{args.stego_method}_{args.batch_size}_{args.lr}'
            if args.suffix != '':
...
if __name__ == '__main__':
    ...
    if args.use_wandb:
        ...
        wandb.save(f'./train_integrated_SupCon.py', policy='now')
